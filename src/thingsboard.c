...
static void coap_decode_response_code(int code, int *class, int *detail)
{
	*class = (code >> 5);
	*detail = code & 0x1f;
}

static void coap_response_code_to_str(int code, char str[5])
{
	int class, detail;
...
int thingsboard_send_timeseries(const thingsboard_timeseries *ts, size_t ts_count)
{
	...
	while ((ts_count - ts_sent) > 0) {
		...
		payload_len[request_no - 1] = buffer_length;

		ts_sent += ts_to_send;
	}

	/* Send all prepared requests */
	for (size_t i = 0; i < request_no; i++) {
